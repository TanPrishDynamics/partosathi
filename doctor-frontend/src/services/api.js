 ...
 *  - PHASE-5 TENANT ISOLATION: every request is registered with the active
 *    AbortController via `attachSessionSignal()`. Calling `cancelAllRequests()`
 *    on logout aborts any in-flight fetch so a response that started under
 *    Doctor A's session cannot land in Doctor B's React tree after a fast
...
// ── Session-scoped AbortController pool (Phase-5 isolation) ─────────────────
let _sessionController = new AbortController();

/**
 * Cancel every in-flight request and start a fresh session.
 * Call this on logout BEFORE clearing user state. Components that started a
 * fetch under the old session receive an AbortError; the cancelled request
 * never resolves into the new session's UI.
 */
export function cancelAllRequests() {
  try { _sessionController.abort(); } catch (_) { /* noop */ }
  _sessionController = new AbortController();
}

api.interceptors.request.use((config) => {
  // Attach the session signal unless the caller passed one explicitly.
  if (!config.signal) config.signal = _sessionController.signal;
  return config;
});
